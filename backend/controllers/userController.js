...
// const payload = { id: profile._id.toString(), email: profile.email };
...
// ✅ Token Generators
const generateAccessToken = (payload) =>
  jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: "100y" });

const generateRefreshToken = (payload) =>
  jwt.sign(payload, process.env.JWT_REFRESH_SECRET, { expiresIn: "100y" });
...
exports.signup = async (req, res) => {
  try {
    ...
    // 4️⃣ Create Profile linked to this AdUser
    ...
    // 5️⃣ Generate tokens with Profile ID
    const payload = { id: profile._id.toString(), email };
    const accessToken = generateAccessToken(payload);
    const refreshToken = generateRefreshToken(payload);
...
exports.login = async (req, res) => {
  try {
    ...
    // ✅ Token with Profile._id (NOT AdUser._id)
    const payload = { id: profile._id.toString(), email: profile.email };
    const accessToken = generateAccessToken(payload);
    const refreshToken = generateRefreshToken(payload);
